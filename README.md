# market-time-line-persuasion
# Brand Archetypes, Design, and methods of Persuasion

## Project Guidelines
-All group members must follow the project template when creating their assigned Markdown files. 
-Each topic must be completed in a separate Markdown file. 
-click here to view the project template: 


## Archetypes 1-12 
-12 types Assignes to Mishal
-Each archetype has a definition, imagery, color scheme
1)The Hero
2)The Magician
3)The Explorer
4)The Outlaw
5)The Sage
6)The Creator
7)The Innocent
8)The Everyperson
9)The Caregiver
10)The Lover
11)The Jester
12)The Ruler
## Methods of Persuasion 1-7 
-7 methods Assigned to Clara
-Each method has a definition, imagery, color scheme
1)Reciprocity
2)Commitment and Consistency
3)Social Proof
4)Authority
5)Liking
6)Scarcity
7)Unity
## Modernism 
-Modernism Assigned to Aryanna
1)Definition 
2)History and origins
3)Main Characteristics
4)Common colors, shapes, and imagery
5)References
## Post Modernism ~Aryanna 
-PostModernism Assigned to Aryanna
1)Definition 
2)History and origins
3)Main Characteristics
4)Common colors, shapes, and imagery
5)References
## Design Styles within modernism and postmodernism 
-Designs of post and modernism Assigned to Tas
-Different designs of modernism and postmodernism with what each represents
-Research 6 modernist and 6 postmodernist design movements.
*suggested modernism movements
1)Bauhaus
2)De Stijl
3)Constructivism
4)Swiss Style
5)Futurism
6)Art Deco

*suggested postmodernism movements
1)Memphis Design
2)New wave typography
3)Deconstructionism
4)Punk Design
5)Postmodern Graphic Design
6)Radical Design
